...
public class ClientDAO {
    ...
    public List<ClientEntity> getListByID(int id) {
        ...
        lsClient = DBUtil.getListHasCondition("role_",id,ClientEntity.class,s);
        return  lsClient;
    }
    public List<ClientEntity> getListByFullname(String fullname) {
        Session s = HibernateUtil.getSession(ClientEntity.class);
        List<ClientEntity> lsClient = new ArrayList<>();
        lsClient = DBUtil.getListByCustomQuery("select * from Client where fullname = N'"+fullname+"' or username ='"+fullname+"'",ClientEntity.class,s);
        return  lsClient;
    }
    public ClientEntity getClientBySecretKey(String fullname) {
        Session s = HibernateUtil.getSession(ClientEntity.class);
       List<ClientEntity> lsClient = DBUtil.getListByCustomQuery("select * from Client where privatekey = N'"+fullname+"'",ClientEntity.class,s);
       if(lsClient.size()>0){
